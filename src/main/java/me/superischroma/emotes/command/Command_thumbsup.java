...
public class Command_thumbsup implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cs, Command cmd, String cmdLabel, String[] args) 
    {
		...
		if (cmdLabel.equalsIgnoreCase("thumbsup")) {
			if (args.length == 0)
            {
                Bukkit.broadcastMessage(((Player) cs).getDisplayName() + ChatColor.GREEN + " gives a thumbs up!");
                return true;
            }
            if (args.length > 1)
            {
            ...
            Bukkit.broadcastMessage(((Player) cs).getDisplayName() + ChatColor.GREEN + " gives a thumbs up to " + p.getDisplayName() + ChatColor.GREEN + "!");
            return true;
		}
        }
        return false;
